...
<div id="controller-panel">
	...
	<div id="controller-panel-right">
		...
		<?php else: ?>
			<a href="#" onClick="loadPage('<?=base_url()?>index.php/periodos_escuelas_controller/add_c','right-content')" id="icon-new" title='Nuevo'>Nuevo</a>
		<?php endif; ?>
		</div>
	</div>
</div>
<div id="form-search">
	<form action="<?=base_url()?>index.php/periodos_escuelas_controller/search_c" method="post" name="formSearchperiodos_escuelas" id="formSearchperiodos_escuelas">
	<fieldset>
		...
		<?php endforeach; ?>
		<p>
		<input type="submit" name="btn-search" id="btn-search" value="Buscar" onClick="submitData('formSearchperiodos_escuelas',new Array('result-list','result-list'))"></input>
		</p>
	</fieldset>
	</form>
</div>
