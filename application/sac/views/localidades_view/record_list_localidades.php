...
<table id="result-set">
	...
	<?php foreach($localidades as $f):?>
		<tr>
			...
			<?php else: ?>
				<td><a href="#" onClick="loadPage('<?=base_url()?>index.php/localidades_controller/edit_c/<?=$f->id?>','right-content')" id="icon-edit">Modificar</a></td>
			<?php endif;?>
			<?php if(!$flag['d']):?>
				<td><div class='block-item'><a href="#" id="icon-delete">Eliminar</a><div class='mask-item'></div></div></td>
			<?php else: ?>
				<td><a href="#" onClick="deleteData('<?=base_url()?>index.php/localidades_controller/delete_c/<?=$f->id?>','right-content','¿Estás seguro de eliminar este item?')" id="icon-delete">Eliminar</a></td>
			<?php endif;?>
		</tr>
	<?php endforeach; ?>
</table>
