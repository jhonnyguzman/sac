				...
				<tbodoy>
					<?php foreach($localidades as $f):?>
						<tr>
							...
							<td>
								<?php if($flag['u']):?>
									<a href="<?=base_url()?>localidades_controller/edit_c/<?=$f->id?>" title="Modificar" class="btn boton_rojo">Modificar</a>
								<?php endif;?>
								<?php if($flag['d']):?>
									<a href="#"  onClick="deleteRow('<?=base_url()?>localidades_controller/delete_c/<?=$f->id?>')" title="Eliminar" class="btn boton_rojo">Eliminar</a>
								<?php endif;?>
							</td>
						</tr>
					<?php endforeach; ?>
