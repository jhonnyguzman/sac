...
class Sys_Perfiles_Controller extends CI_Controller {
	...
	function __construct()
	{
		...
		$this->config->load('sys_perfiles_settings');
	...
	function index()
	{
		...
		if($this->flagR)
		{
			...
			$this->load->view('sys_perfiles_view/home_sys_perfiles', $data);
		}
	}
	...
	function add_c()
	{
		...
		{	
			...
			$id_perfiles = $this->sys_perfiles_model->add_m($data_perfiles);
			if($id_perfiles){ 
				$this->session->set_flashdata('flashConfirm', $this->config->item('flash_add_message')); 
				redirect('sys_perfiles_controller','location');
			}else{
				$this->session->set_flashdata('flashError', $this->config->item('flash_error_message')); 
				redirect('sys_perfiles_controller','location');
			}
		}
		$this->load->view('sys_perfiles_view/form_add_sys_perfiles',$data);
	...
	function edit_c($perfiles_id)
	{
		...
		$data['perfiles'] = $this->sys_perfiles_model->get_m(array('id' => $perfiles_id),$flag=1);
		$data['estados'] = $this->tabgral_model->get_m(array("grupos_tabgral_id" => 1));
		...
		if($this->form_validation->run()){
			...
			if($this->sys_perfiles_model->edit_m($data_perfiles)){ 
		...
		$this->load->view('sys_perfiles_view/form_edit_sys_perfiles',$data);
	...
	{
		//code here
		if($this->sys_perfiles_model->delete_m($perfiles_id)){ 
	...
	function search_c($offset = 0)
	{
		...
		if($this->flagR)
		{
			$fieldSearch = $this->basicrud->getFieldSearch($this->sys_perfiles_model->getFieldsTable_m());
			...
			$data['pagination'] = $this->basicrud->getPagination(array('nameModel'=>'sys_perfiles_model','perpage'=>$this->config->item('pag_perpage')),$data_search_pagination);
			$data['perfiles'] = $this->sys_perfiles_model->get_m($data_search_perfiles);
		
			$data['fieldShow'] = $this->basicrud->getFieldToShow($this->sys_perfiles_model->getFieldsTable_m());
		
			$data['flag'] = $this->flags;

			$this->load->view('sys_perfiles_view/record_list_sys_perfiles',$data);
