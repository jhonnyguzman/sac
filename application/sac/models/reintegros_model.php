...
class Reintegros_Model extends CI_Model {
	...
	{
		//code here
		$this->db->insert('reintegros', $options);
	...
	function get_m($options = array(),$flag=0)
	{
		...
		if(isset($options['monto']))
			$this->db->where('monto', $options['monto']);
		if(isset($options['persona_id']))
			$this->db->where('persona_id', $options['persona_id']);
		if(isset($options['created_at']))
