...
class Horas_institucionales_Model extends CI_Model {
	...
	{
		//code here
		$this->db->insert('horas_institucionales', $options);
	...
	{
		//code here
		if(isset($options['linea_accion_docente_id']))
			$this->db->set('linea_accion_docente_id', $options['linea_accion_docente_id']);
		...
		$this->db->update('horas_institucionales');
	...
	function delete_m($id)
	{
		//code here
		$this->db->where('id', $id);
		$this->db->delete('horas_institucionales');
	...
	function get_m($options = array(),$flag=0)
	{
		...
		if(isset($options['id']))
			$this->db->where('id', $options['id']);
		if(isset($options['linea_accion_docente_id']))
			$this->db->where('linea_accion_docente_id', $options['linea_accion_docente_id']);
		if(isset($options['cantidad_horas']))
			$this->db->where('cantidad_horas', $options['cantidad_horas']);
		if(isset($options['linea_periodo_escuela_id']))
			$this->db->where('linea_periodo_escuela_id', $options['linea_periodo_escuela_id']);
		if(isset($options['created_at']))
		...
		$query = $this->db->get('horas_institucionales');
	...
	function getFieldsTable_m()
	{
		...
		$fields=array();
		$fields[]='id';
		$fields[]='linea_accion_docente_id';
