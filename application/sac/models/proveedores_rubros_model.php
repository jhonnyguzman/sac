...
class Proveedores_rubros_Model extends CI_Model {
	...
	{
		//code here
		$this->db->insert('proveedores_rubros', $options);
	...
	function edit_m($options = array())
	{
		...
		$this->db->update('proveedores_rubros');
	...
	function delete_m($id)
	{
		//code here
		$this->db->where('id', $id);
		$this->db->delete('proveedores_rubros');
	...
	function get_m($options = array(),$flag=0)
	{
		...
		if(isset($options['id']))
			$this->db->where('id', $options['id']);
		if(isset($options['proveedor_id']))
			$this->db->where('proveedor_id', $options['proveedor_id']);
		if(isset($options['rubro_id']))
		...
		$this->db->select("pr.*, p.razon_social as proveedor_razon_social, r.nombre as rubro_nombre");
		$this->db->from("proveedores_rubros as pr");
		$this->db->join("proveedores as p","p.id = pr.proveedor_id");
		$this->db->join("rubros as r","r.id = pr.rubro_id");
