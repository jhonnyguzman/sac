...
class Fuentes_rubros_Model extends CI_Model {
	...
	{
		//code here
		$this->db->insert('fuentes_rubros', $options);
	...
	function get_m($options = array(),$flag=0)
	{
		//code here
		if(isset($options['id']))
			$this->db->where('fr.id', $options['id']);
		if(isset($options['rubro_id']))
			$this->db->where('fr.rubro_id', $options['rubro_id']);
		if(isset($options['fuente_id']))
			$this->db->where('fr.fuente_id', $options['fuente_id']);
		if(isset($options['created_at']))
			$this->db->where('fr.created_at', $options['created_at']);
		if(isset($options['updated_at']))
			$this->db->where('fr.updated_at', $options['updated_at']);
		...
		$this->db->select("fr.*, f.nombre as fuente_nombre, r.nombre as rubro_nombre");
		$this->db->from("fuentes_rubros as fr");
