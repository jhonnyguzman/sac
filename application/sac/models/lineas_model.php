...
class Lineas_Model extends CI_Model {
	...
	function edit_m($options = array())
	{
		...
		if(isset($options['eje_id']))
			$this->db->set('eje_id', $options['eje_id']);
	...
	function get_m($options = array(),$flag=0)
	{
		...
		if(isset($options['descripcion']))
			$this->db->like('l.descripcion', $options['descripcion']);
		if(isset($options['habilitado']))
			$this->db->where('l.habilitado', $options['habilitado']);
		if(isset($options['eje_id']))
			$this->db->where('l.eje_id', $options['eje_id']);
		if(isset($options['created_at']))
		...
		$this->db->select("l.*, e.nombre as eje_nombre");
		$this->db->from("lineas as l");
		$this->db->join("ejes as e","e.id = l.eje_id");
		$query = $this->db->get();
