...
class Proyectos_Model extends CI_Model {
	...
	{
		//code here
		$this->db->insert('proyectos', $options);
	...
	function edit_m($options = array())
	{
		...
		if(isset($options['linea_id']))
			$this->db->set('linea_id', $options['linea_id']);
	...
	function get_m($options = array(),$flag=0)
	{
		...
		if(isset($options['linea_id']))
			$this->db->where('l.linea_id', $options['linea_id']);
		if(isset($options['created_at']))
		...
		$this->db->select("p.*, l.nombre as linea_nombre");
		$this->db->from("proyectos as p");
		$this->db->join("lineas as l","l.id = p.linea_id");
